...
// Use /tmp for Vercel, project dir for local
const DOWNLOAD_DIR = isVercel ? '/tmp/downloads' : path.join(__dirname, '..', 'downloads');
const CACHE_DIR = isVercel ? '/tmp/yt_cache' : path.join(__dirname, '..', 'yt_cache');
const COOKIES_PATH = path.join(__dirname, '..', 'cookies.txt');

// Ensure binaries are executable on Linux (Vercel)
if (process.platform !== 'win32') {
    [YTDLP_PATH, FFMPEG_PATH].forEach(binaryPath => {
        if (fs.existsSync(binaryPath)) {
            try {
                fs.chmodSync(binaryPath, 0o755);
            } catch (e) {
                console.error(`Failed to chmod ${binaryPath}:`, e.message);
            }
        }
    });
}
