...
const { createRequestConfig } = require('./utils.js');
...
/**
 * Utility: Upgrade image quality (smart fallback)
 * Returns upgraded URL but keeps original as fallback
 */
function upgradeImageQuality(url) {
    if (!url) return url;
    // Try to upgrade to originals, but Pinterest may block it
    // The proxy will handle fallback to lower qualities
    // Only upgrade if URL contains a quality specifier
    if (url.match(/\/\d+x\//)) {
...
/**
 * Utility: Get safe image URL (don't force originals)
 * Use this when you want a reliable URL that won't be blocked
 */
function getSafeImageUrl(url) {
    if (!url) return url;
    // If already has originals, downgrade to 564x for reliability
...
async function searchPinterest(keyword, limit = 4, cookie = null) {
    ...
    for (let attempt = 1; attempt <= CONFIG.maxRetries; attempt++) {
        try {
            ...
            const { url: finalUrl, config } = createRequestConfig(searchUrl, {
                headers,
            ...
            $('a[href*="/pin/"]').each((i, el) => {
                ...
                if (href && !processedUrls.has(href)) {
                    ...
                    if (pinId) {
                        ...
                        if (imgSrc) {
                            const highQualityUrl = upgradeImageQuality(imgSrc);
                            const safeThumbnailUrl = getSafeImageUrl(imgSrc);

                            pins.push({
                                id: pinId,
                                url: `https://www.pinterest.com/pin/${pinId}/`,
                                title: img.attr('alt') || `${keyword}`,
                                image: highQualityUrl, // High quality for download
                                thumbnail: safeThumbnailUrl, // Safe quality for display
            ...
            if (pins.length < limit && uniqueImages.length > 0) {
                uniqueImages.forEach((imageUrl, index) => {
                    ...
                    const safeThumbnailUrl = getSafeImageUrl(imageUrl);
...
async function getPinViaSavePin(url) {
    const { url: finalUrl, config } = createRequestConfig(
