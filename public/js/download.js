import { fetchWithRetry, detectPlatform, normalizeUrl } from './utils.js';
import { showPopup } from './ui.js';

// Handle YouTube search
export async function handleYouTubeSearch(keywords) {
  try {
...
export async function handlePinterestSearch(keywords) {
  try {
    const res = await fetch('/api/utils/utility', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function selectPinterestPin(url, title) {
...
export async function selectVideo(url, title) {
  document.getElementById('urlInput').value = url;
...
// Handle URL download
export async function handleUrlDownload(url) {
...
export async function downloadFile(url, filename, proxyUrl = null) {
  try {
    ...
    // Attempt anchor download first (might work for some browsers/configs)
    const a = document.createElement('a');
    a.href = url;
    a.download = filename;
    a.target = '_blank';
...
export async function download(url, format, platform) {
    ...
    if (data.success) {
      ...
      if (downloadUrl) {
        ...
        if (platform === 'TikTok') {
          ...
          // but let's keep the option open or rely on new tab fallback.
...
export async function downloadCarouselItem(mediaUrl, index) {
  const popup = document.getElementById('popup');
...
export async function downloadAllCarousel(mediaUrls) {
  const popup = document.getElementById('popup');
...
export async function downloadTikTokPhotos(url) {
  const popup = document.getElementById('popup');
