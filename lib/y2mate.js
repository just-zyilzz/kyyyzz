 ...
 * @param {string|number} quality - Quality (360, 480, 720, 1080, etc)
 */
function ythd(url, quality = '720') {
    return new Promise((resolve, reject) => {
        if (ytIdRegex.test(url)) {
            let ytId = ytIdRegex.exec(url);
            url = 'https://youtu.be/' + ytId[1];

            // Map quality to y2mate supported values
            const qualityNum = parseInt(quality);
            let y2mateQuality = 720; // default
            if (qualityNum >= 1080) y2mateQuality = 1080;
            else if (qualityNum >= 720) y2mateQuality = 720;
            else if (qualityNum >= 480) y2mateQuality = 480;
            else if (qualityNum >= 360) y2mateQuality = 360;
            else if (qualityNum >= 240) y2mateQuality = 240;
            else y2mateQuality = 144;

            console.log(`[Y2mate] Analyzing: ${url} (quality: ${y2mateQuality})`);
