...
async function XeonInstaMp4(url) {
    try {
        ...
        // Set up VM context for script execution
        let context = {
            i: 0,
            scriptResult: ''
        };
        ...
        vm.runInContext(data, context, { timeout: 5000 });

        // Parse the script result to get download links
        const $ = cheerio.load(context.scriptResult);
        const result = {
            url: [],
            metadata: {}
        };
        ...
        $('a').each(function () {
            ...
            if (href && (href.includes('.jpg') || href.includes('.mp4') || href.includes('cdninstagram'))) {
                result.url.push(href);
            }
        });
