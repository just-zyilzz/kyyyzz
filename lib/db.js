...
function initDB() {
    ...
    try {
        ...
        github_id TEXT UNIQUE
            ...
            // Check if github_id column exists (migration for existing users table)
            db.all("PRAGMA table_info(users)", (err, rows) => {
                if (!err && rows) {
                    const hasGithubId = rows.some(r => r.name === 'github_id');
                    if (!hasGithubId) {
                    ...
                    // Make password nullable if it was not (SQLite doesn't support altering column constraints easily, 
                    // but we can just be careful in code not to require it)
                }
            });
        });
...
function createGithubUser(username, githubId) {
    return new Promise((resolve, reject) => {
        const database = getDB();
        if (!database) return reject(new Error('Database not available'));

        database.run(`INSERT INTO users (username, github_id) VALUES (?, ?)`, [username, githubId], function (err) {
            if (err) reject(err);
            else resolve({ id: this.lastID, username, github_id: githubId });
        });
    });
}
