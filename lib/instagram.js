...
async function downloadInstagram(url) {
    try {
        ...
        if (error.message.includes('login') || error.message.includes('private') || error.message.includes('Sign in to confirm your age')) {
            return {
                status: false,
                code: 403,
                error: 'Cookies tidak valid atau expired. Coba update cookies.txt atau pastikan IP server sama dengan IP saat export cookies.'
            };
        }
        ...
        if (error.message.includes('429') || error.message.includes('Too Many Requests')) {
            return {
                status: false,
                code: 429,
                error: 'Terlalu banyak request (Rate Limit). Coba lagi nanti.'
            };
        }
