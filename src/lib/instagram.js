...
import qs from 'qs';
import { getProxyConfig } from './utils.js';
...
const getDownloadLinks = (url) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // Fix: Scope button selector to current thumbnail item
            $("div.download-items__thumb").each((index, item) => {
                // Find the first download link within THIS thumbnail item
                const link = $(item).find("div.download-items__btn a").first();
                let downloadUrl = link.attr("href");
            ...
            const uniqueUrls = [...new Set(downloadLinks)];

            console.log(`[Instagram Scraper] Found ${uniqueUrls.length} unique URLs from ${downloadLinks.length} total`);
...
async function Instagram(url) {
    ...
    try {
        ...
        console.error('Instagram GraphQL method failed:', e1.message);
        try {
            result = await getDownloadLinks(url);
        } catch (e2) {
            console.error('Instagram Snapsave method failed:', e2.message);
            // Return proper error object instead of { msg: "..." }
            return {
                success: false,
                error: 'Gagal mengambil media Instagram. Pastikan konten bersifat publik dan URL valid. Error: ' + (e2.message || e1.message || 'Unknown error')
            };
        }
    }
    return result;
