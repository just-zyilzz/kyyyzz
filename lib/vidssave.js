...
/**
 * YouTube downloader using Vidssave API with Cloudflare Worker proxy fallback
...
async function ytdl(url) {
    ...
    for (const mirror of mirrors) {
        try {
            const apiUrl = `https://${mirror.domain}/api/contentsite_api/media/parse`;
            ...
            // Use createRequestConfig for automatic Cloudflare Worker routing
            const { url: finalUrl, config } = createRequestConfig(apiUrl, {
                headers: {
            ...
            const res = await axios.post(finalUrl, formData.toString(), config);
